...
class ZoneBody(BaseModel):
    zone_name: str
    description: Optional[str] = None
    is_mustering_zone: bool = False
    max_occupancy: Optional[int] = None   # also accepts legacy "capacity"
    capacity: Optional[int] = None        # kept for backwards compat
    door_ids: List[int] = []   # plain int list — direction defaults to entry (0)

    @property
    def effective_capacity(self):
        return self.max_occupancy if self.max_occupancy is not None else self.capacity


@router.get("/zones/")
async def get_zones(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    zones = db.execute(text("SELECT * FROM acc_zone ORDER BY id")).fetchall()
    result = []
    for z in zones:
        zd = dict(z._mapping)
        doors = db.execute(text("""
            SELECT zd.id AS assignment_id, zd.direction, d.id AS door_id, d.name AS door_name, d.terminal_sn
            FROM acc_zone_door zd JOIN acc_door d ON d.id = zd.door_id
            WHERE zd.zone_id = :zid
        """), {"zid": zd["id"]}).fetchall()
        ...
        if all_door_ids:
            ...
            occ = db.execute(text(f"""
                SELECT COUNT(DISTINCT emp_code) AS cnt FROM (
                    ...
                    FROM acc_event
                    WHERE door_id IN ({ids_sql}) AND emp_code IS NOT NULL AND event_type = 0
                    ORDER BY emp_code, event_time DESC
                ) last WHERE last.in_out = 0
            """)).fetchone()
            zd["current_occupancy"] = int(occ._mapping["cnt"])
        else:
            zd["current_occupancy"] = 0
        result.append(zd)
    return {"success": True, "data": result}
...
async def create_zone(body: ZoneBody, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    row = db.execute(text("""
        INSERT INTO acc_zone (zone_name, description, is_mustering_zone, capacity)
        VALUES (:n, :d, :im, :cap) RETURNING *
    """), {"n": body.zone_name, "d": body.description, "im": body.is_mustering_zone, "cap": body.effective_capacity}).fetchone()
    zid = row._mapping["id"]
    for did in body.door_ids:
        db.execute(text(
...
async def update_zone(zone_id: int, body: ZoneBody, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not db.execute(text("SELECT id FROM acc_zone WHERE id=:id"), {"id": zone_id}).fetchone():
        raise HTTPException(404, "Zone not found")
    db.execute(text("""
        UPDATE acc_zone SET zone_name=:n, description=:d, is_mustering_zone=:im, capacity=:cap WHERE id=:id
    """), {"n": body.zone_name, "d": body.description, "im": body.is_mustering_zone, "cap": body.effective_capacity, "id": zone_id})
    db.execute(text("DELETE FROM acc_zone_door WHERE zone_id=:id"), {"id": zone_id})
    for did in body.door_ids:
        db.execute(text(
            "INSERT INTO acc_zone_door (zone_id, door_id, direction) VALUES (:zid, :did, 0)"
        ), {"zid": zone_id, "did": did})
...
async def delete_zone(zone_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    db.execute(text("DELETE FROM acc_zone_door WHERE zone_id=:id"), {"id": zone_id})
    db.execute(text("DELETE FROM acc_zone WHERE id=:id"), {"id": zone_id})
    db.commit()
    return {"success": True, "message": "Zone deleted"}


@router.get("/zones/{zone_id}/mustering/")
async def zone_mustering(zone_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    """Return list of people currently inside this zone based on last AC event direction."""
    zone = db.execute(text("SELECT * FROM acc_zone WHERE id=:id"), {"id": zone_id}).fetchone()
    if not zone:
        raise HTTPException(404, "Zone not found")
    doors = db.execute(text(
        "SELECT door_id, direction FROM acc_zone_door WHERE zone_id=:zid"
    ), {"zid": zone_id}).fetchall()
    entry_ids = [d._mapping["door_id"] for d in doors if d._mapping["direction"] in (0, 2)]
    if not entry_ids:
        return {"success": True, "data": {"zone": dict(zone._mapping), "present": [], "count": 0}}
    ids_sql = ",".join(str(i) for i in entry_ids)
    rows = db.execute(text(f"""
        SELECT last.emp_code, last.emp_name, last.event_time, last.terminal_sn,
               TRIM(COALESCE(pe.first_name,'') || ' ' || COALESCE(pe.last_name,'')) AS full_name,
               pe.photo
        FROM (
            SELECT DISTINCT ON (emp_code) emp_code, emp_name, event_time, terminal_sn, in_out
            FROM acc_event
            WHERE door_id IN ({ids_sql}) AND emp_code IS NOT NULL AND event_type = 0
            ORDER BY emp_code, event_time DESC
        ) last
        LEFT JOIN personnel_employee pe ON pe.emp_code = last.emp_code
        WHERE last.in_out = 0
        ORDER BY last.event_time DESC
    """)).fetchall()
    data = _rows(rows)
    for item in data:
        if isinstance(item.get("event_time"), datetime):
